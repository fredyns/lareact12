...
interface InputSelectFromResourceProps {
  id: string;
  label: string;
  onChange: (value: string) => void;
  apiEndpoint: string;
  labelField?: string; // Field to use as label (e.g., 'name', 'string')
  labelFormatter?: (item: any) => string; // Custom function to format label
...
export function InputSelectFromResource({
  id,
  label,
  onChange,
  apiEndpoint,
  ...
  const loadOptions = async (inputValue: string): Promise<SelectOption[]> => {
    try {
      ...
      // Handle Laravel Resource Collection format (data.data array)
      const items = result.data || result;
      
      // Transform items to SelectOption format
